...
- **Commit after every change**: once a change passes its Codex review, commit it immediately with a semantic scoped message — never batch a session's work into one end-of-session commit. Docs updated because of a change go in that change's commit; standalone `docs(...)` commits are for doc-only work. Push only when the user asks.
- **Releases**: Claude does the version bumps and writes a brief `CHANGELOG.md` entry for every release, in normal everyday English aimed at app users (Added/Fixed/Improved phrasing, no commit jargon or internal module names). The workflow refuses to publish a tag whose version has no changelog section and uses that section as the Release description. Full flow: `docs/release.md`.
